...
public class ClassUtil {
     ...
     */
    public static List<ClassFiedInfoDto> getClassFieldAndMethod(Class<?> currentClass, boolean isDelete, int gradeNum) throws Exception {
        return getClassFieldAndMethod(currentClass, null, isDelete, gradeNum);
    }
     ...
     *
     * @param currentClass 当前class
     * @param genericClass 泛型的class
     * @param isDelete     是否删除list的缓存
     * @param gradeNum     排序号
     * @return 类的所有字段属性名称（包括父类）
     * @throws Exception 自定异常
     */
    public static List<ClassFiedInfoDto> getClassFieldAndMethod(Class<?> currentClass, Class<?> genericClass, boolean isDelete, int gradeNum)
            throws Exception {
        if (isDelete) {
        ...
        Map<String, String> fieldsRemarkMap = getFieldsRemark(currentClass);// 注释
        Map<String, Map<String,String>> genericfieldsRemarMap= new ConcurrentHashMap<>(); //泛型类的注释
        ...
        for (int i = 0; i < objFields.length; i++) {
            ...
            if ("T".equals(typeName)) {
                //存在泛型
                if(isRealClass(genericClass)){
                    Map<String, String> fieldsRemark = getFieldsRemark(genericClass);
                    genericfieldsRemarMap.put(fieldName, fieldsRemark);
                }
            }
            if (getGetMethod(currentClass, fieldName) != null) {
                fields.add(field);
                fieldNum++;
            }
        }
